...
# 日志文件列表 自定义
log_files = ['/home/hlt/WORK/HYPER/HEMM/logs/20251108_215712.log',  # 1.00: 0.00
            '/home/hlt/WORK/HYPER/HEMM/logs/20251107_131952.log',       # 0.75: 0.25
            '/home/hlt/WORK/HYPER/HEMM/logs/20251107_131828.log',       # 0.50: 0.50
            '/home/hlt/WORK/HYPER/HEMM/logs/20251107_132003.log']       # 0.25: 0.75

# loss_weight值 自定义
loss_weights = [0.0, 0.25, 0.5, 0.75]

# 限定只分析这8个数据集
target_datasets = {'hiv', 'bace', 'bbbp', 'tox21', 'sider', 'clintox', 'toxcast', 'muv'}

# 存储所有数据集的结果
# 数据结构: {dataset_name: {loss_weight: (mean, std)}}
results = {}

# 从日志文件中提取loss_weight值
def extract_loss_weight_from_log(log_content):
    # 查找类似 "✓ 对比损失权重: 0.1" 的行
    match = re.search(r'✓ 对比损失权重: ([0-9.]+)', log_content)
    if match:
...
for i, log_file in enumerate(log_files):
    try:
        ...
        # 提取loss_weight（如果无法从日志中提取，则使用预设值）
        # loss_weight = extract_loss_weight_from_log(content)
        ...
        for dataset_name, (mean_val, std_val) in dataset_results.items():
            ...
            results[dataset_name][loss_weight] = (mean_val, std_val)
...
for dataset_name, dataset_data in results.items():
    ...
    for weight in sorted_weights:
        mean_val, std_val = dataset_data[weight]
        print(f"  loss_weight={weight}: 平均值={mean_val:.4f}, 标准差={std_val:.4f}")

# 保存提取的结果到文件
results_file = os.path.join(save_dir, 'extracted_results.txt')
with open(results_file, 'w', encoding='utf-8') as f:
    ...
    for dataset_name, dataset_data in results.items():
        ...
        for weight in sorted_weights:
            mean_val, std_val = dataset_data[weight]
            f.write(f"  loss_weight={weight}: 平均值={mean_val:.4f}, 标准差={std_val:.4f}\n")
...
for dataset_name, dataset_data in results.items():
    sorted_weights = sorted(dataset_data.keys())
    means = [dataset_data[w][0] for w in sorted_weights]
    stds = [dataset_data[w][1] for w in sorted_weights]
    ...
    plt.xlabel('Loss Weight', fontsize=12, fontweight='bold')
    plt.ylabel('ROC-AUC (Top 3 Average)', fontsize=12, fontweight='bold')
    plt.title(f'{dataset_name.upper()} Dataset Performance', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3, linestyle='--')
    plt.xticks(sorted_weights, fontsize=10)
    plt.yticks(fontsize=10)
    plt.ylim(0.6, 1.0)
    
    # 添加数值标签
    for i, (weight, mean_val) in enumerate(zip(sorted_weights, means)):
        plt.annotate(f'{mean_val:.4f}', (weight, mean_val), textcoords="offset points", 
                    xytext=(0,10), ha='center', fontsize=9, color='black')
    
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'{dataset_name}_performance.png'), dpi=300, bbox_inches='tight')
    plt.close()

# 计算所有数据集的均值和标准差
# 数据结构: {loss_weight: (mean_of_means, std_of_means, mean_of_stds)}
overall_results = {}
sorted_weights = sorted(loss_weights)
...
for weight in sorted_weights:
    ...
    for dataset_name, dataset_data in results.items():
        if weight in dataset_data:
            means_at_weight.append(dataset_data[weight][0])
            stds_at_weight.append(dataset_data[weight][1])
    
    if means_at_weight:
        mean_of_means = np.mean(means_at_weight)
        std_of_means = np.std(means_at_weight)
        mean_of_stds = np.mean(stds_at_weight)
        overall_results[weight] = (mean_of_means, std_of_means, mean_of_stds)

# 创建所有数据集均值的图表
plt.figure(figsize=(10, 7))
sorted_overall_weights = sorted(overall_results.keys())
overall_means = [overall_results[w][0] for w in sorted_overall_weights]
overall_stds_of_means = [overall_results[w][1] for w in sorted_overall_weights]
overall_means_of_stds = [overall_results[w][2] for w in sorted_overall_weights]
...
plt.xlabel('Loss Weight', fontsize=14, fontweight='bold')
plt.ylabel('ROC-AUC (Top 3 Average)', fontsize=14, fontweight='bold')
plt.title('Overall Performance Across All Datasets', fontsize=16, fontweight='bold')
plt.legend(fontsize=11, frameon=True, fancybox=True, shadow=True, framealpha=0.9)
plt.grid(True, alpha=0.4, linestyle=':', linewidth=0.8)
plt.xticks(sorted_overall_weights, fontsize=12)
...
for idx, (dataset_name, dataset_data) in enumerate(results.items()):
    ...
    for weight in sorted_weights:
        if weight in dataset_data:
            means.append(dataset_data[weight][0])
            stds.append(dataset_data[weight][1])
...
plt.xlabel('Loss Weight', fontsize=14, fontweight='bold')
plt.ylabel('ROC-AUC (Top 3 Average)', fontsize=14, fontweight='bold')
plt.title('Performance Comparison Across All Datasets', fontsize=16, fontweight='bold')
...
# 设置坐标轴刻度
plt.xticks(sorted_weights, fontsize=12)
plt.yticks(fontsize=12)
...
# 调整布局以适应图例
plt.tight_layout()
plt.savefig(os.path.join(save_dir, 'all_datasets_performance.png'), dpi=300, bbox_inches='tight')
plt.close()

print(f"\n图表已生成并保存到 {save_dir} 目录:")
print("1. 每个数据集的单独图表已保存为 {dataset_name}_performance.png")
print("2. 所有数据集的对比图表已保存为 all_datasets_performance.png")
print("3. 所有数据集均值图表已保存为 overall_performance.png")