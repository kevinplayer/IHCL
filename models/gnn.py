...
class ViSNet(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 mlp_hidden_dim=128):
        super().__init__()
        self.backbone = ViSNet(lmax=lmax, hidden_channels=hidden_channels, num_layers=num_layers)
    '''

class Drug2(nn.Module):
    def __init__(self, lmax, num_heads, num_layers, hidden_channels, drug_features_path=None):
        ...
        # 加载预计算的3D药物特征
        if drug_features_path and os.path.exists(drug_features_path):
            self.drug_features = torch.load(drug_features_path)
            print(f"Loaded 3D drug features from {drug_features_path}")
        else:
            self.drug_features = None
            print("No 3D drug features file provided, will compute on-the-fly")
        
        # 药物特征缓存
        self.drug_cache = {}
        self.cache_limit = 10000  # 缓存大小限制

    def get_drug_features_3d(self, drug_name, smiles):
        """获取3D药物特征，优先使用缓存和预计算特征"""
        # 检查缓存
        if drug_name in self.drug_cache:
            return self.drug_cache[drug_name]
        
        # 检查预计算特征
        if self.drug_features and drug_name in self.drug_features.get('ik2graph3D', {}):
            x, z, pos, edge_index, edge_attr = self.drug_features['ik2graph3D'][drug_name]
            result = (x, z, pos, edge_index, edge_attr)
        # else:
        #     pdb.set_trace()
        #     # 实时计算3D特征
        #     print(f"Computing 3D drug features for {drug_name}")
        #     try:
        #         # 收集所有原子类型并创建types字典
        #         all_atoms = set()
        ...
        #         # 按照原子序数排序创建types字典
        #         sorted_atoms = sorted(all_atoms, key=lambda x: Chem.Atom(x).GetAtomicNum())
        #         types = {atom: idx for idx, atom in enumerate(sorted_atoms)}
                
        #         result = smile_to_graph_xyz(smiles, types)
        #     except Exception as e:
        #         print(f"Error computing 3D features for {drug_name}: {e}")
        #         # 如果计算失败，返回默认值
        #         result = (torch.empty(0), torch.empty(0), torch.empty(0), torch.empty(0), torch.empty(0))
        
        # 缓存结果（如果缓存未满）
        if len(self.drug_cache) < self.cache_limit:
            self.drug_cache[drug_name] = result
    ...
    def forward(self, data):
        ...
        # 获取3D药物特征
        if hasattr(data, 'drug_name') and hasattr(data, 'smiles'):
            # 批量处理
            batch_size = len(data.drug_name)
            z_list = []
            pos_list = []
            batch_list = []
            
            for i in range(batch_size):
                x, z, pos, edge_index, edge_attr = self.get_drug_features_3d(
                    data.drug_name[i], data.smiles[i]
                )
                ...
                pos_list.append(pos)
                # 调整边索引以适应批量处理
                batch_list.extend([i] * x.size(0))
            
            # 合并批量数据
            z_batch = torch.cat(z_list, dim=0)
            pos_batch = torch.cat(pos_list, dim=0)
            batch_tensor = torch.tensor(batch_list, dtype=torch.long).to(device)
            
        else:
            # 原始处理方式（兼容性）
            x_batch, z_batch, pos_batch, edge_index_batch, batch_tensor = data.x, data.z, data.pos, data.edge_index, data.batch
            z_batch = z_batch.to(device)
            pos_batch = pos_batch.to(device)
            batch_tensor = batch_tensor.to(device)

        # 使用ViSNet处理3D分子图
        d_feature = self.visnet(z_batch, pos_batch, batch_tensor)
        
        return d_feature

# ------------------ Graph Transformer ------------------
class GraphTrans(nn.Module):
    def __init__(self, input_drug_feature_dim=9, use_drug_edge=True, input_drug_edge_dim=3,
                 fc_1_dim=256, fc_2_dim=128, dropout=0.00, transformer_dropout=0.00):
    ...
    def forward(self, data):
        ...
        # 获取药物特征
        x, edge_index, batch = data.raw_x, data.raw_edge_index, data.batch
        x = x.to(device)
        edge_index = edge_index.to(device)
        batch = batch.to(device)
        if self.use_drug_edge:
            edge_attr = data.raw_edge_attr
    ...
    def get_mol_embedding(self, data):
        device = next(self.parameters()).device
        # 获取药物特征
        x, edge_index, batch = data.raw_x, data.raw_edge_index, data.batch
        x = x.to(device)
